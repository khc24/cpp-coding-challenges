...
"apeach"			0
"neo"				2
위 예시에서는 2번 이상 신고당한 "frodo"와 "neo"의 게시판 이용이 정지됩니다. 이때, 각 유저별로 신고한 
아이디와 정지된 아이디를 정리하면 다음과 같습니다.
...
정지 기준이 되는 신고 횟수 k가 매개변수로 주어질 때, 각 유저별로 처리 결과 메일을 받은 횟수를 배열에 
...
vector<int> solution(vector<string> id_list, vector<string> report, int k) {
    ...
    unordered_map<string, int> map_dic;
    ...
    for (const auto& id : id_list)
    {
        answer.push_back(map_dic[id]);
    }
    return answer;
}

// 해설지 풀이: 더 간결함
vector<int> solution_commentay(vector<string> id_list, vector<string> report, int k) {
